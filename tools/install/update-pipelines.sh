...
getPipelineRepoGitUri(){
    ...
    _response=$( request_get ${_uri}) 
    PIPELINE_REPO_GIT_HTTP_URL=$(echo $_response | jq -c -r '.remoteUrl')
    PIPELINE_REPO_ID=$(echo $_response | jq -c -r '.id')
    _debug "CODE_REPO_GIT_HTTP_URL: $PIPELINE_REPO_GIT_HTTP_URL"
    _debug "CODE_REPO_ID: $PIPELINE_REPO_ID"
    _information "${PIPELINE_REPO_NAME} Git Repo remote URL: "$PIPELINE_REPO_GIT_HTTP_URL
}
...
syncPipelinesRepo(){
    ...
    cd ${WORK_DIR}/Terraform-Pipelines
    git remote set-url origin ${PIPELINE_REPO_GIT_HTTP_URL} 
